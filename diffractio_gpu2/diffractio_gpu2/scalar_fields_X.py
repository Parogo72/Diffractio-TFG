...
from .kernels.rs_kernel_X import RS_X_gpu
...
class Scalar_field_X():
    ...
    def RS_pycuda(self, z, amplification=1, n=1.0, kind='z',
              xout=None, new_field=True, matrix=False,
        ...
        t2 = time.time()
        if verbose:
            print(f"RS_pycuda X ({precision}) = {t2-t1:.4f}s")
        ...
        if new_field:
            ...
            field_out   = Scalar_field_X(x=x0, wavelength=self.wavelength)
            field_out.u = u_out
            return field_out
        else:
            self.x = x0
