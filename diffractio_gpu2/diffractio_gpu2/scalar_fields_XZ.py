...
# Always use NumPy and CPU FFT for XZ
import numpy as np
...
from .kernels.rs_kernel_XZ import RS_XZ_gpu
...
def _to_cpu(values):
    """Return a CPU array when a GPU-like object exposes .get()."""
...
class Scalar_field_XZ():
    ...
    def __RS_multiprocessing__(self, i: int):
        ...
        # Convert to CPU if kernel returned GPU array
        H = _to_cpu(H)
    ...
    def RS(self, xout: float | None = None, verbose: bool = False,
        ...
        b = array(a * num_rep)
        W = concatenate((array([1]), b, array([2, 1]))) / 3.
    ...
    def RS_pycuda(self, xout=None, verbose=False, precision='float32'):
        t1 = time.time()

        x_out = self.x if xout is None else xout
        ...
        t2 = time.time()
        if verbose:
            print(f"RS_pycuda XZ ({precision}) = {t2-t1:.4f}s")
        return t2 - t1
