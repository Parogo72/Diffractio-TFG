...
from .kernels.rs_kernel_XY import RS_XY_gpu
...
# flake8: noqa
    ...
    def RS_pycuda(self, z, amplification=(1,1), n=1.0, kind='z',
              xout=None, yout=None, new_field=True, matrix=False,
        ...
        t2 = time.time()
        if verbose:
            print(f"RS_pycuda XY ({precision}) = {t2-t1:.4f}s")
        ...
        if new_field:
            ...
            field_out   = Scalar_field_XY(x=X0, y=Y0, wavelength=self.wavelength)
            field_out.u = U
            return field_out
        else:
            self.u = U
