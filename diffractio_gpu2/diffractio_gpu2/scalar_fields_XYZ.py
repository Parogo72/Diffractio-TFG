...
from .kernels.rs_kernel_XYZ import RS_XYZ_gpu
...
class Scalar_field_XYZ():
    ...
    def incident_field(self, u0, z0: float | None = None):
        ...
        if u0.x.shape == self.x.shape:
            ...
            if z0 in (None, '', []):
                self.u[:, :, 0] = self.u[:, :, 0] + u0_u

            else:
                iz, _, _ = nearest(self.z, z0)
                self.u[:, :, iz] = self.u[:, :, iz] + u0_u
    ...
    def RS_pycuda(self, verbose: bool = False):
        time1 = time.time()

        if verbose:
            print(f"Launching RS_pycuda: grid={self.u0.u.shape}, Nz={len(self.z)}")
        ...
        elapsed = time2 - time1
        if verbose:
            print(f"RS_pycuda completed in {elapsed:.4f}s")
