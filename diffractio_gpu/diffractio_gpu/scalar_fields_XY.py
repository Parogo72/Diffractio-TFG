...
def _rect_bivariate_spline_backend(x_values, y_values, z_values, *args, **kwargs):
    spline_cpu = _RectBivariateSpline(_to_cpu(x_values), _to_cpu(y_values), _to_cpu(z_values), *args, **kwargs)
    if not _CUPY_BACKEND:
        return spline_cpu

    def _wrapped(x_new, y_new, **call_kwargs):
        return _to_backend(spline_cpu(_to_cpu(x_new), _to_cpu(y_new), **call_kwargs))
...
# flake8: noqa
    ...
    def cut_resample(self,
        ...
        if num_points not in ([], '', 0, None):
            ...
            f_interp_abs = _rect_bivariate_spline_backend(self.y, self.x, np.abs(self.u), kx=kxu, ky=kxu, s=0)
            f_interp_phase = _rect_bivariate_spline_backend(self.y, self.x, np.angle(self.u), kx=kxu, ky=kxu, s=0)
