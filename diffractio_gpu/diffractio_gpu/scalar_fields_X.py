...
try:
    import cupy as np
    from cupyx.scipy.fft import fft, fftshift, ifft
    try:
        from cupyx.scipy.special import hankel1
    except Exception:
        from scipy.special import hankel1 as _scipy_hankel1

        def hankel1(v, z):
            return np.asarray(_scipy_hankel1(v, np.asnumpy(z)))
    _CUPY_BACKEND = True
except Exception:
    import numpy as np
...
from .__init__ import degrees, mm, plt
...
def _interp1d_backend(x_values, y_values, *args, **kwargs):
    interp_cpu = _scipy_interp1d(_to_cpu(x_values), _to_cpu(y_values), *args, **kwargs)
    if not _CUPY_BACKEND:
        return interp_cpu

    def _wrapped(x_new):
        return _to_backend(interp_cpu(_to_cpu(x_new)))
...
class Scalar_field_X():
    ...
    def cut_resample(
        ...
        if num_points not in empty_types:
            ...
            f_interp_abs = _interp1d_backend(
                self.x,
                np.abs(self.u),
                kind=interp_kind,
                bounds_error=False,
            ...
            f_interp_phase = _interp1d_backend(
                self.x,
                np.imag(self.u),
                kind=interp_kind,
                bounds_error=False,
    ...
    def insert_mask(self, t1, x0_mask1: float, clean: bool = True, kind_position: str = "left"):
        ...
        f_interp_real = _interp1d_backend(
            t1.x, np.real(t1.u), kind="nearest", bounds_error=False, fill_value=0
        )

        f_interp_imag = _interp1d_backend(
    ...
    def _RS_(self, z: float, n: float, matrix: bool = False,
        ...
        x_values = _to_backend(self.x)

        if xout is None:
            xout = x_values[0]

        xout = _to_backend(xout)

        xout = x_values + xout - x_values[0]
        ...
        xext = x_values[0] - xout[::-1]  # da la vuelta
        xext = xext[0:-1]
        xext = concatenate((xext, x_values - xout[0]))
        if z > 0:
