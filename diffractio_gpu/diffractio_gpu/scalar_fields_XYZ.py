...
from .__init__ import degrees, mm, plt, num_max_processors
...
from .scalar_fields_XY import PWD_kernel, Scalar_field_XY, WPM_schmidt_kernel, kernelRS, kernelRSinverse
...
def _to_cpu(values):
    if _CUPY_BACKEND and isinstance(values, np.ndarray):
        return np.asnumpy(values)
    return values


def _to_backend(values):
    if _CUPY_BACKEND:
        return np.asarray(values)
    return values


def _regular_grid_interpolator_backend(points, values, *args, **kwargs):
    points_cpu = tuple(_to_cpu(axis_values) for axis_values in points)
    interpolator_cpu = _RegularGridInterpolator(points_cpu, _to_cpu(values), *args, **kwargs)
    if not _CUPY_BACKEND:
        return interpolator_cpu

    def _wrapped(xi, *call_args, **call_kwargs):
        xi_cpu = xi
        if isinstance(xi, tuple):
            xi_cpu = tuple(_to_cpu(item) for item in xi)
        elif isinstance(xi, np.ndarray):
            xi_cpu = _to_cpu(xi)
        return _to_backend(interpolator_cpu(xi_cpu, *call_args, **call_kwargs))
...
class Scalar_field_XYZ():
    ...
    def __init__(self, x: NDArrayFloat | None = None, y: NDArrayFloat | None = None,
        ...
        if x is not None and y is not None and z is not None:
            self.x = _to_backend(x)
            self.y = _to_backend(y)
            self.z = _to_backend(z)
        else:
            self.x = x
            self.y = y
            self.z = z
        ...
        if self.x is not None and self.y is not None and self.z is not None:
            ...
            self.u0 = Scalar_field_XY(self.x, self.y, wavelength)
    ...
    def xy_2_xyz(self, u0_XY, z: NDArrayFloat):
        ...
        """
        u0 = u0_XY[0]
        self.x = _to_backend(u0.x)
        self.y = _to_backend(u0.y)
        self.z = _to_backend(z)
    ...
    def cut_resample(self,
        ...
        if num_points not in ([], '', 0, None):
            ...
            f_interp_amplitude = _regular_grid_interpolator_backend(
                (self.x, self.y, self.z), np.abs(self.u))
            f_interp_phase = _regular_grid_interpolator_backend((self.x, self.y, self.z),
                                                     np.angle(self.u))
            ...
            n_interp_real = _regular_grid_interpolator_backend((self.x, self.y, self.z),
                                                    np.real(self.n))
            n_interp_imag = _regular_grid_interpolator_backend((self.x, self.y, self.z),
                                                    np.imag(self.n))
    ...
    def RS_batched(self,
                   ...
                   kind: str = 'z',
                   verbose: bool = False):
        """Rayleigh-Sommerfeld propagation using batched FFTs over z planes.

        This method is equivalent to calling ``self.u0.RS(..., matrix=True)`` for each
        z position, but it reuses invariant terms and computes several z planes at once.
        It is especially useful when CuPy backend is active.

        Args:
            batch_size (int): Number of z planes computed per batch.
            n (float | None): Refractive index. If None, ``self.n_background`` is used.
            kind (str): Kernel component ('z', 'x', 'y', '0').
        ...
        U[0:ny, 0:nx] = self.u0.u
        FU = fft2(U)

        xext = x0[0] - x0[::-1]
        xext = xext[0:-1]
        xext = np.concatenate((xext, x0 - x0[0]))

        yext = y0[0] - y0[::-1]
        yext = yext[0:-1]
        yext = np.concatenate((yext, y0 - y0[0]))

        Xext, Yext = np.meshgrid(xext, yext)

        num_z = len(self.z)
        for i0 in range(0, num_z, batch_size):
            i1 = min(i0 + batch_size, num_z)
            z_chunk = self.z[i0:i1]
            local_batch = i1 - i0

            Xb = Xext[None, :, :]
            Yb = Yext[None, :, :]
            zb = z_chunk[:, None, None]

            H = np.empty((local_batch, 2 * ny - 1, 2 * nx - 1), dtype=complex)

            pos_mask = z_chunk > 0
            neg_mask = ~pos_mask
            if np.any(pos_mask):
                H[pos_mask] = kernelRS(Xb, Yb, self.wavelength, zb[pos_mask], n=n, kind=kind)
            if np.any(neg_mask):
                H[neg_mask] = kernelRSinverse(Xb, Yb, self.wavelength, zb[neg_mask], n=n, kind=kind)

            FH = fft2(H, axes=(-2, -1))
            S = ifft2(FH * FU[None, :, :], axes=(-2, -1)) * dx * dy
            self.u[:, :, i0:i1] = np.transpose(S[:, ny - 1:, nx - 1:], (1, 2, 0))

            if verbose is True:
                print(f"RS batched: {i1}/{num_z}", end='\r')

        time2 = time.time()
        if verbose is True:
            print()
        print(("time in RS_batched= {}. batch_size= {}".format(time2 - time1,
                                                                batch_size)))
        return time2 - time1
